...
if [ "$#" -lt 3 ] ; then
	...
	for cfg in `ls $execpath/cfg/surfmorph.*.cfg` ; do
		cfg=${cfg##*/surfmorph.}
		cfg=${cfg%%.cfg}
		echo "         $cfg"
	done
...
if [ -n "$surfmorph_type" ] ; then
	if [ -e $execpath/cfg/surfmorph.$surfmorph_type.cfg ] ; then
		surfmorph_cfg=$execpath/cfg/surfmorph.$surfmorph_type.cfg 
	elif [ -e $surfmorph_type ] ; then 
...
if [  -n "$in_seg_dir" -a -n "$matching_seg" -a -n "$seg_name" ] ; then # if specified
	if [ ! -e $in_seg_dir ] ; then
	...
	if [ ! -n "$template_prob_seg" -o  ! -n "$template_prob_seg_file" ] ; then
...
if ! test -h $work_folder/$atlas ; then
	if test -d $work_folder/$atlas ; then
...
if [ "$analysis_level" = "group1" ] ; then
	#if 1st template doesn't exist, then create it:
	if [ ! -e $work_folder/${seg_name}_iter1/template_prob_seg.nii.gz ] ; then
		...
		for subj in $subjlist ; do
			...
			for subjfolder in `ls -d $in_bids/$subj/anat $in_bids/$subj/ses-*/anat 2> /dev/null` ; do
				subj_sess_dir=${subjfolder%/anat}
				subj_sess_dir=${subj_sess_dir##$in_bids/}

				if echo $subj_sess_dir | grep -q '/' ; then
					sess=${subj_sess_dir##*/}
				...
				#import T1 from BIDS
				if [ "$skip_bet" = "0" ] ; then
					if [ ! -e $work_folder/$subj_sess_prefix/t1/t1.nii.gz ] ; then
				...
				if [ "$skip_bet" = "1" ] ; then
					...
					if [ ! -e $work_folder/$subj_sess_prefix/t1/t1.brain.nii.gz ] ;	then
						N_t1w=`eval ls $in_bids/$subj_sess_dir/anat/${subj_sess_prefix}${searchstring_t1w} | wc -l`
						in_t1w=`eval ls $in_bids/$subj_sess_dir/anat/${subj_sess_prefix}${searchstring_t1w} | head -n 1`
				...
				if false ; then
					if [ "$use_atlasseg" = "1" ] ; then
						#generating segmentations using atlas-based segmentation
						pushd $work_folder

						#performing registration
						if [ ! -e $subj_sess_prefix/reg/bspline_f3d_t1/${atlas}_${subj_sess_prefix}/ctrlpt_${atlas}_to_${subj_sess_prefix}.nii.gz ] ; then
							echo reg_bspline_f3d t1 $atlas $subj_sess_prefix
							reg_bspline_f3d t1 $atlas $subj_sess_prefix
						fi

						if [ ! -e ${subj_sess_prefix}/$target_prob_seg ] ; then
							#propagating labels
				...
				if [ -e $work_folder/${subj_sess_prefix}/reg/affine_aladin_t1/${atlas}_${subj_sess_prefix}/${subj_sess_prefix}_to_${atlas}.xfm ] ; then
		...
		if [ -n "${subj_sess_prefix_reg}" ] ; then
			pushd $work_folder
			echo "parallel echo $execpath/deps/reg_intersubj_aladin  t1 $atlas {} -I $execpath/deps/init.xfm ::: ${subj_sess_prefix_reg}"
			parallel echo $execpath/deps/reg_intersubj_aladin  t1 $atlas {} ::: ${subj_sess_prefix_reg} #-I $execpath/deps/init.xfm 
			parallel $execpath/deps/reg_intersubj_aladin  t1 $atlas {} ::: ${subj_sess_prefix_reg} #-I $execpath/deps/init.xfm 
			popd
		fi
	...
	for iter in `seq 1 $MAX_ITER` ; do
		if [ ! -e $work_folder/${seg_name}_iter$iter/template_prob_seg.nii.gz ] ; then
			buildTemplate $iter
...
elif [ "$analysis_level" = "participant1" ] ; then
	echo "analysis level participant1 - run template to target mapping"

	#figure out which iteration we are on
	for iter in `seq 1 $MAX_ITER` ; do
		if [ -e $work_folder/${seg_name}_iter$iter/template_prob_seg.nii.gz -a ! -e $work_folder/${seg_name}_iter$((iter+1))/template_prob_seg.nii.gz ] ; then
	...
	#loop over sub- and sub-/ses-
		for subjfolder in `ls -d $in_bids/$subj/anat $in_bids/$subj/ses-*/anat 2> /dev/null` ; do
			...
			iter_out=$work_folder/${seg_name}_iter$iter
			if [ ! -e $work_folder/$iter_out/${subj_sess_prefix}/templateSurface_${seg_name}_inout.vtk ] ; then
